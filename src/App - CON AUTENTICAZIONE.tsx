...
import { isAuthenticated, logout, refreshSession } from './utils/auth';
...
const App: React.FC = () => {
  ...
  const handleLogout = () => {
    logout();
    setIsLoggedIn(false);
    setView('dashboard'); // Reset to dashboard on logout
  };
  ...
  const handleSearch = async (query: string) => {
      ...
      try {
          ...
          setSearchResults({ customers: [], reports: [], appointments: [] });
      }
    ...
    switch (view) {
      ...
      case 'search_results':
        return <SearchResultsView
                ...
                />;
      case 'settings':
        return <SettingsView profile={technicianProfile} onSave={handleSaveProfile} onLogout={handleLogout} />;
      default:
        return <DashboardView 
                    customers={customers} 
                    reports={reports} 
